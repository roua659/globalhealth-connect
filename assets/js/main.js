...
(function() {
    ...
    function initChatbot() {
        ...
        window.sendMessage = async function() {
            ...
            const message = input.value.trim();
            if (!message || chatbotRequestInFlight) return;

            addChatMessage(message, 'user');
            input.value = '';

            if (!window.chatbotApiUrl) {
                addChatMessage("Le service IA n'est pas configure sur cette page.", 'bot');
                return;
            }

            chatbotHistory.push({ role: 'user', content: message });
            chatbotRequestInFlight = true;
            setChatInputState(true);
            const pendingMessage = addChatMessage('Assistant IA en cours de reponse...', 'bot');

            try {
                const response = await fetch(window.chatbotApiUrl, {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok || !result.success) {
                    addChatMessage(`Erreur IA : ${result.message || "Le service IA a retourne une erreur."}`, 'bot');
                    return;
                }

                const reply = String(result.reply || '').trim();
                if (!reply) {
                    addChatMessage("Le service IA n'a renvoye aucune reponse.", 'bot');
                    return;
                }
                ...
                if (pendingMessage) pendingMessage.remove();
                addChatMessage(`Impossible de joindre le service IA : ${error.message}`, 'bot');
            } finally {
                chatbotRequestInFlight = false;
        ...
        function addChatMessage(message, type) {
            ...
            const messageDiv = document.createElement('div');
            messageDiv.className = `message ${type}`;
            messageDiv.innerHTML = escapeChatHtml(String(message || '')).replace(/\n/g, '<br>');
            messages.appendChild(messageDiv);
            messages.scrollTop = messages.scrollHeight;
            return messageDiv;
        }

        function setChatInputState(isDisabled) {
            const input = document.getElementById('chatInput');
