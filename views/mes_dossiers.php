...
<body>
    <div class="page-shell">
        ...
        <div id="dossierAlert" class="alert top-alert"></div>
        ...
        <div class="panel-card p-4 p-md-5 mb-4">
            ...
            <form id="medicalRecordForm" method="POST" action="../index.php?page=dossiers&action=create" enctype="multipart/form-data">
                ...
                <input type="hidden" name="id_patient" id="medicalPatientId" value="">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="medicalPatientDisplay" value="" placeholder="Connectez-vous comme patient" readonly>
                    </div>
                    <div class="col-md-6">
        ...
        <div class="panel-card p-4 p-md-5">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge bg-light text-primary" id="medicalDossierCount">0 dossier(s)</span>
            </div>
            <div id="medicalDossiersContainer"></div>
        </div>
    </div>
    ...
    <script>
        ...
        function showDossierAlert(message, type = 'warning') {
            const alert = document.getElementById('dossierAlert');
        ...
        const medicalPatientId = document.getElementById('medicalPatientId');
        const medicalPatientDisplay = document.getElementById('medicalPatientDisplay');
        const medicalRdv = document.getElementById('medicalRdv');
        const medicalDossiersContainer = document.getElementById('medicalDossiersContainer');
        const medicalDossierCount = document.getElementById('medicalDossierCount');
        ...
        function resetMedicalForm() {
            ...
            if (medicalRdv) {
                medicalRdv.innerHTML = '<option value="">Aucun</option>';
            }
        ...
        function renderMedicalDossiers(dossiers) {
            if (!medicalDossiersContainer) return;

            medicalDossierCount.textContent = `${dossiers.length} dossier(s)`;

            if (!Array.isArray(dossiers) || dossiers.length === 0) {
                medicalDossiersContainer.innerHTML = `
                    <div class="empty-box">
                                ...
                                <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
                                    <div>
                                        <div class="d-flex flex-wrap align-items-center gap-2 mb-2">
                                            <h2 class="h5 mb-0">${escapeHtml(dossier.diagnostic || 'Dossier medical')}</h2>
                                            <span class="badge bg-light text-primary">${escapeHtml(dossier.date_creation || '')}</span>
                                        </div>
                                        <div class="dossier-meta mb-2">
                                            <i class="fas fa-user-doctor me-2"></i>
                                            Dr. ${escapeHtml(dossier.medecin_nom || '')}
                                        </div>
                                        ${dossier.date_rdv ? `
                                            <div class="dossier-meta mb-2">
                                                <i class="fas fa-calendar-check me-2"></i>
                                                ${escapeHtml(`${dossier.date_rdv || ''} ${(dossier.heure_rdv || '').substring(0, 5)}`)}
                                            </div>
                                        ` : ''}
                                        ${dossier.symptomes ? `<p class="mb-2"><strong>Symptomes :</strong> ${nl2brSafe(dossier.symptomes)}</p>` : ''}
                                        ${dossier.traitement ? `<p class="mb-2"><strong>Traitement :</strong> ${nl2brSafe(dossier.traitement)}</p>` : ''}
                                        ${dossier.ordonnance_texte ? `<p class="mb-2"><strong>Ordonnance :</strong> ${nl2brSafe(dossier.ordonnance_texte)}</p>` : ''}
                                        ${dossier.ordonnance_fichier ? `
                                            <p class="mb-2">
                                                <a class="btn btn-sm btn-outline-primary" href="../uploads/${encodeURIComponent(dossier.ordonnance_fichier)}" target="_blank">
                                                    <i class="fas fa-file-arrow-down me-2"></i>Voir le fichier
                                                </a>
                                            </p>
                                        ` : ''}
                                        ${dossier.historique_modification ? `<div class="history-box mt-3">${escapeHtml(dossier.historique_modification)}</div>` : ''}
                                    </div>
                                    <div class="action-stack d-flex flex-column gap-2">
                                        <button
                                            type="button"
                                            class="btn btn-edit-dossier"
                                            data-dossier='${escapeHtml(JSON.stringify({
                                                id_dossier: Number(dossier.id_dossier || 0),
                                        ...
                                        <form method="POST" action="../index.php?page=dossiers&action=delete" onsubmit="return confirm('Supprimer ce dossier medical ?');">
                                            <input type="hidden" name="id_dossier" value="${Number(dossier.id_dossier || 0)}">
                                            <input type="hidden" name="redirect_to" value="views/mes_dossiers.php">
            ...
            document.querySelectorAll('[data-dossier]').forEach(button => {
                button.addEventListener('click', function() {
                    const raw = this.getAttribute('data-dossier');
                    if (!raw) return;
                    editMedicalRecord(JSON.parse(raw));
                });
            });
        }
        ...
        async function loadMedicalContext() {
            ...
            if (!currentPatient) {
                showDossierAlert('Connectez-vous comme patient pour voir vos dossiers medicaux.');
                if (medicalRecordForm) medicalRecordForm.style.display = 'none';
                if (medicalDossiersContainer) medicalDossiersContainer.innerHTML = '';
                return;
            }

            if (!currentPatient.id_patient) {
                showDossierAlert('Cette page est reservee aux patients.');
                if (medicalRecordForm) medicalRecordForm.style.display = 'none';
                if (medicalDossiersContainer) medicalDossiersContainer.innerHTML = '';
                return;
            }

            if (medicalPatientId) medicalPatientId.value = currentPatient.id_patient;
            if (medicalPatientDisplay) medicalPatientDisplay.value = currentPatient.name || '';

            try {
                const [rdvResponse, dossierResponse] = await Promise.all([
                    fetch(`../index.php?page=rendezvous&action=getByPatient&id_patient=${encodeURIComponent(currentPatient.id_patient)}`),
                    fetch(`../index.php?page=dossiers&action=getByPatient&id_patient=${encodeURIComponent(currentPatient.id_patient)}`)
                ]);
                ...
                if (medicalRdv) {
                    ...
                    if (Array.isArray(rendezVous)) {
                        rendezVous.forEach(rdv => {
                            ...
                            medicalRdv.appendChild(option);
                        });
                    }
                }

                renderMedicalDossiers(Array.isArray(dossiers) ? dossiers : []);
            } catch (error) {
                showDossierAlert('Impossible de charger vos dossiers medicaux.', 'danger');
                if (medicalDossiersContainer) medicalDossiersContainer.innerHTML = '';
            }
        }

        document.addEventListener('DOMContentLoaded', loadMedicalContext);
    </script>
</body>
</html>
