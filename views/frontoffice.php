...
try {
    ...
    if (!empty($defaultPatient['id_patient'])) {
        $rdvController = new RendezVousController();
        $dossierController = new DossierMedicalController();
        $rendezVousPatient = $rdvController->getByPatient($defaultPatient['id_patient']);
        $patientDossiers = $dossierController->getByPatient($defaultPatient['id_patient']);
    }

    if ($frontofficePdo) {
        try {
            $stmt = $frontofficePdo->prepare("SELECT * FROM publication WHERE statut = 'approved' ORDER BY date_publication DESC LIMIT 20");
            $stmt->execute();
            $publications = $stmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Exception $e) {
            try {
                $stmt = $frontofficePdo->prepare("SELECT * FROM publication ORDER BY date_publication DESC LIMIT 20");
                $stmt->execute();
                $publications = $stmt->fetchAll(PDO::FETCH_ASSOC);
            ...
            <div class="col-lg-8">
                <div class="review-form-container">
                    ...
                    <form id="appointmentForm" method="POST" action="../index.php?page=rendezvous&action=create" novalidate>
                        <input type="hidden" name="id_patient" id="frontAppointmentPatientId" value="<?php echo (int)($defaultPatient['id_patient'] ?? 0); ?>">
                        <input type="hidden" name="redirect_to" value="views/frontoffice.php#consultation">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label-medical">Patient</label>
                                <input type="text" class="form-control form-control-medical" id="patientName" value="<?php echo htmlspecialchars(trim(($defaultPatient['prenom'] ?? '') . ' ' . ($defaultPatient['nom'] ?? ''))); ?>" readonly>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label-medical">Email</label>
                                <input type="email" class="form-control form-control-medical" id="patientEmail" value="<?php echo htmlspecialchars($defaultPatient['email'] ?? ''); ?>" readonly>
                            </div>
                            ...
                            <div class="col-12">
                                <div class="alert alert-info mb-0">Le frontoffice utilise le patient de test deja present dans la base.</div>
                            </div>
                            <div class="col-md-6">
    ...
    <div class="container">
            ...
            <form id="medicalRecordForm" method="POST" action="../index.php?page=dossiers&action=create" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control form-control-medical" value="<?php echo htmlspecialchars(trim(($defaultPatient['prenom'] ?? '') . ' ' . ($defaultPatient['nom'] ?? '')) . ' (ID: ' . (int)($defaultPatient['id_patient'] ?? 0) . ')'); ?>" readonly>
                        <input type="hidden" name="id_patient" id="id_patient" value="<?php echo (int)($defaultPatient['id_patient'] ?? 0); ?>">
                    </div>
                    <div class="col-md-6">
        ...
        <div class="medical-folder-card">
            <div class="d-flex justify-content-between align-items-center mb-3">
                ...
                <span class="badge-medical" id="recordCount"><?php echo count($patientDossiers); ?> dossier(s)</span>
            </div>
            ...
            <?php if(count($publications) > 0): ?>
                <?php foreach($publications as $pub): 
                    ...
                    if(!empty($pub['id_medecin']) && $frontofficePdo) {
                        try {
                            $stmt = $frontofficePdo->prepare("SELECT nom, prenom FROM utilisateur WHERE id_user = ?");
...
window.frontofficeLegacyApiAvailable = false;
</script>
...
function populateFrontofficeDoctors() {
    ...
    ['followupDoctor', 'id_medecin', 'reviewDoctorId', 'pubFormDoctor'].forEach(selectId => {
        ...
        doctors.forEach(doctor => {
            ...
            option.value = doctor.id_medecin;
