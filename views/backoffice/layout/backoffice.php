...
try {
    ...
    $columnCheck = $pdoInit->query("SHOW COLUMNS FROM publication LIKE 'statut'");
    if ($columnCheck && $columnCheck->rowCount() === 0) {
        $pdoInit->exec("ALTER TABLE publication ADD COLUMN statut ENUM('approved','blocked') NOT NULL DEFAULT 'approved'");
    }
} catch (Exception $e) {
}
...
if ($action && ($_SERVER['REQUEST_METHOD'] === 'POST' || $_SERVER['REQUEST_METHOD'] === 'GET')) {
    ...
    if ($action === 'add-publication') {
        try {
            ...
            echo json_encode($result['success']
                ? ['success' => true, 'message' => 'Publication cr????e avec succ??s', 'id' => $result['id']]
    ...
    if ($action === 'admin-update-comment-status') {
        try {
            ...
            $pdo = config::getConnexion();
            $pdo->prepare("UPDATE commentaire SET statut = ? WHERE id_commentaire = ?")->execute([$input['statut'], (int) $input['id']]);
            echo json_encode(['success' => true]);
            exit;
        } catch (Exception $e) {
            echo json_encode(['success' => false, 'error' => $e->getMessage()]);
...
<script>
    ...
    function filterCommentsTable() {
        ...
        if (tbody) tbody.innerHTML = filtered.map(renderCommentRow).join('') || '<tr><td colspan="7" class="text-center text-muted py-3">Aucun résultat</td></tr>';
    }
