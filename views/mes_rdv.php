...
<body>
    <div class="page-shell">
        ...
        <div id="rdvAlert" class="alert top-alert"></div>
        <div id="rdvContainer"></div>
    </div>
            ...
            <div class="modal-content border-0" style="border-radius: 24px;">
                ...
                <form id="editRdvForm" method="POST" action="../index.php?page=rendezvous&action=update">
                    <div class="modal-body">
                        ...
                        <input type="hidden" name="id_patient" id="editRdvPatientId" value="">
    ...
    <script>
        ...
        function showRdvAlert(message, type = 'warning') {
            const alert = document.getElementById('rdvAlert');
        ...
        async function loadPatientAppointments() {
            ...
            if (!currentPatient) {
                showRdvAlert('Connectez-vous comme patient pour voir vos rendez-vous.');
                container.innerHTML = '';
                return;
            }

            if (!currentPatient.id_patient) {
                showRdvAlert('Cette page est reservee aux patients.');
                container.innerHTML = '';
                return;
            }

            document.getElementById('editRdvPatientId').value = currentPatient.id_patient;

            try {
                const response = await fetch(`../index.php?page=rendezvous&action=getByPatient&id_patient=${encodeURIComponent(currentPatient.id_patient)}`);
                ...
                container.innerHTML = `
                    <div class="row g-4">
                        ${rendezVous.map(rdv => {
                            ...
                            const consultationLabel = rdv.type_consultation === 'video' ? 'Teleconsultation' : 'Consultation presentielle';
                                        ...
                                        <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
                                            <div>
                                                ...
                                                <div class="rdv-meta mb-2">
                                                    ...
                                                    Dr. ${escapeHtml(rdv.medecin_nom || 'Medecin')}
                                                    ${rdv.specialite ? ` - ${escapeHtml(rdv.specialite)}` : ''}
                                                </div>
                                                <div class="rdv-meta mb-2">
                                                ...
                                                <div class="rdv-meta">
                                                    <i class="fas fa-stethoscope me-2"></i>${consultationLabel}
                                                </div>
                                            </div>
                                            <div class="rdv-actions d-flex flex-column gap-2">
                                                <a
                                                    class="btn btn-outline-primary btn-delete-rdv w-100"
                                                    href="../index.php?page=rendezvous&action=addGoogleCalendar&id=${Number(rdv.id_rdv)}&redirect_to=views/mes_rdv.php"
                                                >
                                                    <i class="fab fa-google me-2"></i>Google Agenda
                                                </a>
                                                ${!isConfirmed ? `
                                                    <button
                                                        type="button"
                                                    ...
                                                    <form method="POST" action="../index.php?page=rendezvous&action=delete" onsubmit="return confirm('Supprimer ce rendez-vous ?');">
                                                        <input type="hidden" name="id_rdv" value="${Number(rdv.id_rdv)}">
                                                        <input type="hidden" name="redirect_to" value="views/mes_rdv.php">
                ...
                showRdvAlert('Impossible de charger vos rendez-vous.', 'danger');
                container.innerHTML = '';
            }
        }
        ...
        if (editRdvModal) {
            editRdvModal.addEventListener('show.bs.modal', async function(event) {
                ...
                if (!rdvId || !currentPatient?.id_patient) return;

                try {
                    const response = await fetch(`../index.php?page=rendezvous&action=getOne&id=${rdvId}`);
                    const data = await response.json();

                    if (Number(data.id_patient || 0) !== Number(currentPatient.id_patient)) {
                        throw new Error('Ce rendez-vous ne correspond pas au patient connecte.');
                    }
